...
export function micro(pl, iso, offset) {
  ...
  return {
    ...
    kb: inline([[btn('🍬 מסטיק ✓', 'g'), btn('🌊 יש לי דחף עכשיו', 'sos:1')]]),
  };
}
...
export function noon(pl, iso, day, meta) {
  ...
  const lines = [
    ...
    `🌊 דחפים שעברו היום: <b>${day.surfed}</b>`,
    '',
  ...
  return {
    ...
    kb: inline([
      [btn('🍬 מסטיק ✓', 'g'), btn('🌊 דחף שעבר +1', 'sf')],
      [btn('🌊 יש לי דחף עכשיו', 'sos:1'), btn('🧰 כלים', 'T:menu')],
    ]),
  };
}
...
export function risk(pl, iso, day, meta) {
  ...
  return {
    ...
    kb: inline([
      ...
      [btn('🌊 יש לי דחף עכשיו', 'sos:1'), btn('🍬 מסטיק ✓', 'g')],
    ]),
  };
}
...
export function evening(pl, iso, day, meta, isSaturdayNight) {
  ...
  parts.push(`   🌀 דחפים שהיו: <b>${day.waves}</b> · 🌊 עברו עד הסוף: <b>${day.surfed}</b>`);
  ...
  const rows = [
    [btn('🌀 היה דחף +1', 'wv'), btn('🌊 דחף שעבר +1', 'sf')],
...
export function status(pl, iso, day, meta) {
  ...
  parts.push(`🌊 דחפים שעברו עד הסוף: <b>${t.surfed}</b>  <i>(היום: ${day.surfed})</i>`);
  parts.push(`🌀 דחפים שהיו: <b>${t.waves}</b>  <i>(היום: ${day.waves})</i>`);
  ...
  return {
    ...
    kb: inline([
      ...
      [btn('🌊 דחף שעבר +1', 'sf'), btn('🧰 כלים', 'T:menu')],
    ]),
  };
}
...
export function outing(pl, iso, day, meta, ilNow) {
  ...
  return {
    ...
    kb: inline([
      ...
      [btn('🌊 יש דחף / אני בדרך לחנות', 'sos:1')],
...
export function help() {
  return {
    text: [
      ...
      '',
      `<b>גם מילות מפתח עובדות:</b> "יוצא מהבית" · "יש לי דחף" · "בא לי" · "קניתי" · "מסטיק"`,
      '',
...
export function welcome(pl, meta) {
  ...
  return {
    text: [
      ...
      `• רגע דחף — כתוב "יש לי דחף" או /גל ואני מוביל אותך צעד-צעד ב-RAIN.`,
      `• יוצא מהבית — כתוב "יוצא מהבית" ואני נותן את טקס 20 השניות ואת מה שרלוונטי לשעה הזאת.`,
      `• כל מסטיק, כל דחף שעבר, כל ניצחון — נספרים. <b>מודדים שחרורים, לא רק ימים.</b>`,
      '',
...
export function partnerInfo(meta, code) {
  if (meta.partnerChatId) {
    const auto = !meta.partnerMute;
    return {
      text: [
        `👥 <b>שותף/ה מחובר/ת ✓</b>`,
        LINE,
        `<b>דיווח אוטומטי: ${auto ? 'מופעל ✅' : 'כבוי 🔇'}</b>`,
        auto
          ? `<b>🧠 המחשבות מחפשות דרך לקנות</b> ו-<b>🆘 אני בדרך לקנות</b> שולחים לה דיווח <b>מיד</b>, בלי צעד נוסף. <b>🌊 דחף רגיל — לא.</b> וכשהדחף עובר, היא מקבלת גם את זה.`
          : `כרגע לא נשלח לה כלום אוטומטית. אפשר לשלוח ידנית בכפתור למטה.`,
        '',
        `<i>הדיווח יוצא על 🧠 ועל 🆘 — <b>תמיד ואוטומטית</b>. דחף רגיל (🌊) לא מגיע אליה.</i>`,
        `<i>ההגנה היחידה היא מפני לחיצה כפולה: אותה דרגה פעמיים בתוך 90 שניות נשלחת פעם אחת. אין חסימה ארוכה מזה — עדיף שתקבל פעמיים מאשר שלא תקבל.</i>`,
        '',
      ...
      kb: inline([
        [btn(auto ? '🔇 כבה דיווח אוטומטי' : '✅ הפעל דיווח אוטומטי', 'pr:auto')],
        [btn('📨 שלח דיווח עכשיו', 'pr:send')],
        [btn('🔌 ניתוק', 'pr:off')],
      ]),
    };
  }
  return {
    text: [
      ...
      '',
      `אחרי החיבור זה <b>אוטומטי</b>: לחיצה על "יש לי דחף עכשיו" שולחת לה מיד, בלי צעד נוסף.`,
      '',
